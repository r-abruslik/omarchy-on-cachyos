...
# FIX 1: Removed unnecessary 'shift'
safe_write() {
  local target="$1"
  cat > "$target" || die "Failed to write $target"
}

# FIX 3: Added die on failure
...
# FIX 2: Proper escaping for grep
...
if ! sudo pacman -Syu --noconfirm; then
  echo "  ⚠ pacman -Syu failed; continuing, but Omarchy repo may be out of date" >&2                                             fi
echo "  ✓ Repository configured"
                                                                                                                                 # User config
echo ""
echo ">> User configuration"                                                                                                     echo "Username:"
read -r OMARCHY_USER_NAME
export OMARCHY_USER_NAME                                                                                                         echo "Email:"
read -r OMARCHY_USER_EMAIL
export OMARCHY_USER_EMAIL
# SDDM autologin
echo ""                                                                                                                          echo ">> Configuring SDDM autologin..."
run sudo mkdir -p /etc/sddm.conf.d
sudo tee /etc/sddm.conf.d/autologin.conf > /dev/null <<EOF                                                                       [Autologin]
User=$OMARCHY_USER_NAME
Session=hyprland                                                                                                                 EOF
echo "  ✓ Autologin configured"
                                                                                                                                 # CachyOS patches
echo ""
echo ">> Applying CachyOS compatibility patches..."                                                                              [[ ! -f "install.sh" ]] && die "install.sh not found"

# FIX 4: Make critical patches hard-fail (no || true)                                                                            # Kernel detection for CachyOS
if [[ -f "bin/omarchy-update-restart" ]]; then
  sed -i \\                                                                                                                          -e "s/ | sed 's\\/-arch\\/\\\\.arch\\/'//" \\
    -e "s/'{print \\$2}'/'{print \\$2 \\\"-\\\" \\$1}' | sed 's\\/-linux\\/\\/'//" \\
    -e "/linux-cachyos/ ! s/pacman -Q linux/pacman -Q linux-cachyos/" \\                                                             bin/omarchy-update-restart || die "failed to patch bin/omarchy-update-restart"
fi
                                                                                                                                 # Remove tldr (tealdeer conflict)
if [[ -f "install/omarchy-base.packages" ]]; then
  sed -i '/tldr/d' install/omarchy-base.packages || die "failed to remove tldr package"                                          fi

# Disable conflicting scripts                                                                                                    if [[ -f "install/preflight/all.sh" ]]; then
  sed -i '/run_logged \\$OMARCHY_INSTALL\\/preflight\\/pacman\\.sh/d' install/preflight/all.sh || \\
    die "failed to patch install/preflight/all.sh"                                                                               fi

if [[ -f "install/config/all.sh" ]]; then                                                                                          sed -i '/run_logged \\$OMARCHY_INSTALL\\/config\\/hardware\\/nvidia\\.sh/d' install/config/all.sh || \\
    die "failed to patch install/config/all.sh"
fi
if [[ -f "install/login/all.sh" ]]; then
  sed -i \\                                                                                                                          -e '/run_logged \\$OMARCHY_INSTALL\\/login\\/plymouth\\.sh/d' \\
    -e '/run_logged \\$OMARCHY_INSTALL\\/login\\/limine-snapper\\.sh/d' \\
    -e '/run_logged \\$OMARCHY_INSTALL\\/login\\/alt-bootloaders\\.sh/d' \\                                                          install/login/all.sh || die "failed to patch install/login/all.sh"
fi
...
fi                                                                                                                                                                                                                                                                # Patch mise for Fish/Bash
if [[ -f "config/uwsm/env" ]]; then                                                                                                cp config/uwsm/env config/uwsm/env.bak                                                                                           if grep -q "omarchy-cmd-present mise" config/uwsm/env; then
    sed -i '/omarchy-cmd-present mise.*activate bash/c\\                                                                         if [ "$SHELL" = "/bin/bash" ] && command -v mise &> /dev/null; then\\                                                              eval "$(mise activate bash)"\\
elif [ "$SHELL" = "/bin/fish" ] && command -v mise &> /dev/null; then\\                                                            mise activate fish | source\\                                                                                                  fi' config/uwsm/env || die "failed to patch config/uwsm/env"
  fi                                                                                                                             fi                                                                                                                               echo "  ✓ Patches applied"
                                                                                                                                 # Copy to local                                                                                                                  echo ""
echo ">> Copying to ~/.local/share/omarchy..."
mkdir -p ~/.local/share/omarchy                                                                                                  cp -r . ~/.local/share/omarchy
cd ~/.local/share/omarchy || die "cannot cd ~/.local/share/omarchy"
echo "  ✓ Copied"

# Backup existing Hyprland config
echo ""
echo ">> Backing up existing config..."                                                                                          if [[ -d ~/.config/hypr ]]; then
  cp -r ~/.config/hypr ~/.config/hypr.backup.$(date +%s)
  echo "  ✓ Backed up (~/.config/hypr.backup.<timestamp>)"                                                                       else
...
windowrulev2 = stayfocused, tag:jetbrains                                                                                        windowrulev2 = noborder, tag:jetbrains
windowrulev2 = size >50% >50%, class:(^jetbrains-.*), title:(^$), floating:1
windowrulev2 = noinitialfocus, class:(^jetbrains-.*), title:(^win.*), floating:1
windowrulev2 = nofocus, class:(^jetbrains-.*), title:(^win.*)                                                                    EOF
...
# Fix 4: pip.conf
safe_write "$HYPR_APPS/pip.conf" <<'EOF'
