...
if lspci | grep -i nvidia > /dev/null 2>&1; then
    ...
    if pacman -Q nvidia-open-dkms &> /dev/null || pacman -Q nvidia-utils &> /dev/null; then
        ...
        if [[ "$SWITCH_TO_PROPRIETARY" =~ ^[Yy]$ ]]; then
            ...
            # Laptop detection (before reboot message)
echo " >> checking chassis type..."
CHASSIS_TYPE=$(cat /sys/devices/virtual/dmi/id/chassis_type 2>/dev/null || echo 0)

# Chassis types 8-11 correspond to portable devices (Laptop, Notebook, Handheld, etc.)
if [[ "$CHASSIS_TYPE" =~ ^(8|9|10|11)$ ]]; then
    ...
    # 1. Shell profile workaround (for terminal apps)
...
EOF
    ...
    # 2. Systemd environment generator (CRITICAL for GUI apps/D-Bus services)
...
EOF
    ...
    # Desktop optimization: Force VA-API to use NVIDIA
